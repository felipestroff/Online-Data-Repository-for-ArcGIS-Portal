...
var GeoShape = {
     ...
     */
    transformAndDownload: function (jsonObject, resultZipFileName) {
        ...
        for (var fname in this.files) {
            ...
            if (f.records.length > 0) {
                ...
                if (jsonObject.crs && jsonObject.crs.properties && jsonObject.crs.properties.name)
                    zip.file(f.getPrjName(resultZipFileName), jsonObject.crs.properties.name);
            }
        }

        if (!resultZipFileName)
            resultZipFileName = "shapefile.zip";
...
class ESRIFileGen {
     ...
     * Target files - names
     */
    getShpName(layerName) {
        return layerName + ".shp";
    }
    getShxName(layerName) {
        return layerName + ".shx";
    }
    getDbfName(layerName) {
        return layerName + ".dbf";
    }
    getCpgName(layerName) {
        return layerName + ".cpg";
    }
    getCsvName(layerName) {
        return layerName + ".csv";
    }
    getPrjName(layerName) {
        return layerName + ".prj";
    }
    /**
