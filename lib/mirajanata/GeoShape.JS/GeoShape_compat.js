...
var GeoShape = {
     ...
     * @param {any} resultZipFileName - desired filename of output zip file
     */
    transformAndDownload: function transformAndDownload(jsonObject, resultZipFileName) {
        this.files = [];
        ...
        for (var fname in this.files) {
            ...
            if (f.records.length > 0) {
                f.writeShp();
                zip.file(f.getShpName(resultZipFileName), f.shp);
                f.writeShx();
                zip.file(f.getShxName(resultZipFileName), f.shx);
                f.writeDbf();
                zip.file(f.getDbfName(resultZipFileName), f.dbf);
                zip.file(f.getCpgName(resultZipFileName), "UTF-8");
                if (f.useCSV) {
                    f.writeCsv();
                    zip.file(f.getCsvName(resultZipFileName), f.csv);
                }

                if (jsonObject.crs && jsonObject.crs.properties && jsonObject.crs.properties.name) zip.file(f.getPrjName(resultZipFileName), jsonObject.crs.properties.name);
            }
        }

        if (!resultZipFileName) resultZipFileName = "result.zip";
        zip.generateAsync({ type: "blob" }).then(function (blob) {
            // 1) generate the zip file
            GeoShape._downloadBlob(blob, resultZipFileName + '.zip'); // 2) trigger the download
        }, function (err) {
...
var ESRIFileGen = function () {
    ...
    _createClass(ESRIFileGen, [{
        ...
        key: "getShpName",
        value: function getShpName(name) {
            return name + ".shp";
        }
    }, {
        key: "getShxName",
        value: function getShxName(name) {
            return name + ".shx";
        }
    }, {
        key: "getDbfName",
        value: function getDbfName(name) {
            return name + ".dbf";
        }
    }, {
        key: "getCpgName",
        value: function getCpgName(name) {
            return name + ".cpg";
        }
    }, {
        key: "getCsvName",
        value: function getCsvName(name) {
            return name + ".csv";
        }
    }, {
        key: "getPrjName",
        value: function getPrjName(name) {
            return name + ".prj";
        }
        /**
